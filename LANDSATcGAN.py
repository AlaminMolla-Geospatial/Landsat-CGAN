...
OUTPUT_DIR = '/home/amolla/DNN/DL_Fall25/FinalProject_EEE598/LandsatData'
...
# -----------------------------
# 4. Detect all L1-L2 pairs (robust)
# -----------------------------
...
def l1_key(filename):
    ...
    return f"{parts[0]}_{parts[2]}_{parts[3]}"  # satellite + pathrow + date
...
for p in pairs: print(p[0].split('/')[-1], "<-->", p[1].split('/')[-1])
...
# All metrics across all pairs
all_metrics = []

# -----------------------------
# 6. Training loop
# -----------------------------
for pair_idx, (l1_path,l2_path) in enumerate(pairs):
    print(f"\nTraining on pair {pair_idx+1}/{len(pairs)}: {os.path.basename(l1_path)}")
    dataset = PairedPatchDataset([(l1_path,l2_path)], CONFIG['in_bands'], CONFIG['out_band'],
    ...
    # -----------------------------
    # 7. Visualization: n_visual_patches per pair (random)
    # -----------------------------
    indices = random.sample(range(len(dataset)), CONFIG['n_visual_patches'])
    for i, idx in enumerate(indices):
        xb_patch, yb_patch, lo_patch, hi_patch = dataset[idx]
        ...
        all_metrics.append([f'pair{pair_idx+1}_patch{i+1}', mae, rmse])
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR,f'pair{pair_idx+1}_patch{i+1}_visualization.png'), dpi=300)
        plt.close()

# -----------------------------
# 8. Save metrics CSV (single file)
# -----------------------------
csv_file = os.path.join(OUTPUT_DIR,'all_pairs_selected_patches_metrics.csv')
with open(csv_file, mode='w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['Patch','MAE(K)','RMSE(K)'])
...
print("Training, visualization, metrics, and loss curve saving complete.")
